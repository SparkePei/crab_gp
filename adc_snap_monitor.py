...
# create ideal gaussian shape for 8 bits
	...
	plt.title('Histogram IF0',bbox=dict(facecolor='red', alpha=0.5))
	...
	#plt.ylim(0,1000)
	plt.xlabel('ADC sample bins.')
	plt.title('Histogram IF1',bbox=dict(facecolor='red', alpha=0.5))
	#plt.hist(inter_analog, bins = 256, range = (-128,128))
	...
	plt.plot(freqs/1e6,spectrum_b,'r')
	#plt.legend()
